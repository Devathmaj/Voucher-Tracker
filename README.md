...
| **3** | **Configure your environment variables.** Complete every guide in the [Environment Setup Guides](#environment-setup-guides) section below to obtain all required API keys, database credentials, and other configuration values. Populate your local `.env` file with the values from those guides. |
...
| **Supabase** | **PostgreSQL database** for the project — includes creating a project, retrieving your database connection string, and connecting it via `DATABASE_URL`. | [Supabase Setup](./docs/setup/supabase-setup.md) |
| **Resend** | Sends **email notifications** — includes generating an API key and setting up the required email environment variables. | [Resend Setup](./docs/setup/resend-setup.md) |
...
> **Complete these guides** before moving on to running or testing the project.
...
| [UptimeRobot Setup](./docs/setup/uptime-bot-setup.md) *(optional)* | Prevent your Render free tier service from spinning down by setting up a monitor that periodically pings your app. |
...
| [Configuration](./docs/details/configuration.md) | A full reference of all available configurations. |
